...
Initialize_Database().then(async init => {
	...
	const Middleware = {
		...
		notification: async (req, res, next) => {
			if (req.params.chat_id !== undefined) {
	...
	.get('/post/:slug?', Middleware.chat, async (req, res) => {
		if (req.params.slug == undefined) {
			...
			post = post.map((element, index) => {
				var images_tag = element.content.match(/<img [^>]*src="[^"]*"[^>]*>/ig);
				images_tag.map((el, key) => {
					var regex_src = new RegExp(/((src=")(.*)("))/, 'i');
					var image_src = regex_src.exec(el);
					if (image_src.length >= 4) {
						element.image_preview = image_src[3];
					}
				});
	...
	.post('/chat_bot/:option?/:id?', async (req, res) => {
		if (req.params.option == 'start') {
			...
			io.of('/').emit('chat-room-end', req.params.id);
	 ...
	 * Live Chat Module
	 */
	.get('/admin/live_chat/:chat_id?', Middleware.admin, Middleware.notification, Middleware.chats, async (req, res) => {
